...
class Module extends _core.Component {
  ...
  addListeners() {
    ...
    if (this.el.dataset.videoAutoPlay === 'true') {
      ...
      if (this.youtubePlayer === this.el.dataset.videoPlayer) {
        return this.dom.backgroundVideo.appendChild(this.getYoutubeIframe(true, 1));
      }
  ...
  getYoutubeIframe() {
    let controls = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : false;
    let autoplay = arguments.length > 1 ? arguments[1] : undefined;
    const iframe = document.createElement('iframe');
    iframe.setAttribute('width', 640);
    iframe.setAttribute('height', 360);
    iframe.setAttribute('frameborder', 0);
    iframe.setAttribute('src', `https://www.youtube.com/embed/${this.videoId}?modestbranding=1&rel=0&showinfo=0&autohide=1&autoplay=${autoplay}&mute=1${controls ? '&controls=0' : ''}`);
    return iframe;
  }
  ...
  getVimeoIframe(autoplay) {
    ...
    iframe.setAttribute('src', `https://player.vimeo.com/video/${this.videoId}?title=false&autoplay=${autoplay}&muted=1`);
    return iframe;
  }
